...
/**
 * Options for creating a useDataManagement mock
 */
export interface MockDataManagementHookOptions {
  clearAllData?: ReturnType<typeof vi.fn>;
  isClearing?: boolean;
}

/**
 * Creates a useDataManagement hook mock return value
 *
 * @example
 * const mockClearAllData = vi.fn();
 * vi.mock("@/hooks/use-data-management", () => ({
 *   useDataManagement: () => createMockDataManagementHook({ clearAllData: mockClearAllData }),
 * }));
 */
export function createMockDataManagementHook(options: MockDataManagementHookOptions = {}) {
  return {
    clearAllData: options.clearAllData ?? vi.fn().mockResolvedValue(undefined),
    isClearing: options.isClearing ?? false,
  };
}

/**
 * Options for creating a useConversationBackup mock
 */
export interface MockConversationBackupHookOptions {
...
/**
 * Creates a useConversationBackup hook mock return value
 *
 * @example
 * const mockExportConversations = vi.fn();
 * vi.mock("@/hooks/use-conversation-backup", () => ({
 *   useConversationBackup: () => createMockConversationBackupHook({ exportConversations: mockExportConversations }),
 * }));
 */
